...
import sys
sys.path.append('./grand/src/')
from grand.src.GNN_early import GNNEarly
...
class GraphUnet(nn.Module):
    ...
    def __init__(self, ks, drop_p, dataset, sml_dict):
        ...
        self.bottom_gcn = GNNEarly(opt=self.sml_dict, dataset=self.dataset, num_features=dim)
        ...
        for i in range(self.l_n):
            gcn = GNNEarly(opt=self.sml_dict, dataset=self.dataset, num_features=lst[i])
            self.down_gcns.append(gcn)
            gcn = GNNEarly(opt=self.sml_dict, dataset=self.dataset, num_features=up_lst[-i - 1])
    ...
    def to_edge_index(self, adj, device='cpu'):
    ...
    def forward(self, g, h):
        ...
        for i in range(self.l_n):
            ...
            curr_edge_index = self.to_edge_index(adj)
...
def top_k_graph(scores, g, h, k, rw='self-loop', device='cpu'):
    num_nodes = g.shape[0]
...
def train(model, edge_index, x, supervision,  y, lf, optmz, device='cpu'):
